...
export default function EmergencyPage() {
  ...
  // initialize socket and map once
  useEffect(() => {
  ...
  const showAmbulanceList = () => {
    ...
    if (ambulances[0]) map.setView([ambulances[0].lat, ambulances[0].lng], 11)
  }

  // --- WebRTC / Call logic (simplified from provided code) ---
  ...
  // booking ambulance: share current location with ambulance service via socket and show on map
  const bookAmbulance = (ambId: string) => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      // emit booking event
      socketRef.current?.emit('ambulance-book', { ambulanceId: ambId, userLocation: { latitude, longitude } })
      ...
      if (map) {
        ...
        userMarker.current = L.marker([latitude, longitude], { icon: L.icon({ iconUrl: 'https://unpkg.com/leaflet@1.9.3/dist/images/marker-icon.png', iconSize: [25, 41], iconAnchor: [12, 41] }) }).addTo(map)
        userMarker.current.bindPopup('Your location').openPopup()
        if (selected) map.setView([(latitude + selected.lat) / 2, (longitude + selected.lng) / 2], 12)
      }
    }, (err) => {
    ...
    <div style={{ width: '100%' }}>
      ...
      <div className="emergency-container">
        <div className="emergency-buttons">
          <button className="video-btn" onClick={() => showDoctors('video')}>🎥 Video Call</button>
          <button className="audio-btn" onClick={() => showDoctors('audio')}>📞 Audio Call</button>
          <button className="ambulance-btn" onClick={showAmbulanceList}>🚑 Need Ambulance?</button>
        </div>

        <div style={{ flexGrow: 1 }}>
          {showDoctorList && selectedDoctors.map((doc, index) => (
            <div key={index} className="item">
              <strong>{doc.name}</strong><br />
              Specialization: {doc.specialization}<br />
              Location: {doc.location}<br />
              <button style={{ marginTop: 6, padding: '6px 12px', border: 'none', borderRadius: 10, background: '#34d399', color: 'white', cursor: 'pointer' }} onClick={() => connectDoctor(doc.name, 'video')}>📞 Call Now</button>
            </div>
          ))}
          ...
          {showAmbulances && ambulances.map((a) => (
            <div key={a.id} className="item">
              ...
              <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                <button style={{ padding: '8px 12px', borderRadius: 8, background: '#ef4444', color: 'white', border: 'none' }} onClick={() => bookAmbulance(a.id)}>Book Ambulance</button>
                <a style={{ padding: '8px 12px', borderRadius: 8, background: '#2563eb', color: 'white', textDecoration: 'none' }} href={`tel:${a.phone}`}>Call</a>
              </div>
            </div>
          ))}
      ...
      {callModal && (
        <div className="call-modal">
          <div className="call-box">
            <h3>{callDoctor}</h3>
            <div>{callStatus}</div>
            <div id="video-room" ref={videoRoomRef} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginTop: 10 }}></div>
            <div style={{ marginTop: 15 }}>
              <button style={{ padding: '10px 20px', background: '#ef4444', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer' }} onClick={endCall}>End Call</button>
            </div>
          </div>
        </div>
      )}

      <footer style={{ marginTop: 40, background: '#1e40af', color: 'white', width: '100%', padding: 15, textAlign: 'center' }}>
        © 2025 PHN System | <a style={{ color: '#93c5fd', margin: '0 10px' }} href="/privacy">Privacy Policy</a> | <a style={{ color: '#93c5fd', margin: '0 10px' }} href="/terms">Terms of Service</a>
      </footer>
    </div>
  )
}
